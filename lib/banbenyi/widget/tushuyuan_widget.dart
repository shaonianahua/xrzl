...
//图书管理员组件
class TushuyuanWidget extends StatefulWidget {
  const TushuyuanWidget({super.key});

  @override
  State<TushuyuanWidget> createState() => _TushuyuanWidgetState();
}

class _TushuyuanWidgetState extends State<TushuyuanWidget> {
      ...
      children: [
        Text(
          indexController.roleMap[roleId]['isSelect'] &&
                  !(indexController.roleMap[roleId]['false'] ?? false)
              ? "图书管理员操作：选择两个人告知有一个人是xx外来者身份"
              : "图书管理员操作：本场并无图书管理员 请表演：图书管理员请睁眼，他与他其中一个人是这个身份 图书管理员请闭眼",
        ...
        GestureDetector(
            onTap: () {
              if (indexController.roleMap[roleId]['isSelect'] &&
                  ...
                  name2 != "" &&
                  role != 0) {
                indexController.journal(
                    "图书管理员得知了$name1和$name2中存在${XConfig.roleMap[role]}");
